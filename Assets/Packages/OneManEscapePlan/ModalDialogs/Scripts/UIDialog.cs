...
namespace OneManEscapePlan.ModalDialogs.Scripts {
	...
	public class UIDialog : UIDialogBase {
		...
		/// <param name="settings"></param>
		virtual protected void ApplyFontSettings(Text text, FontSettings settings) {
			Assert.IsNotNull(text);
			Assert.IsNotNull(settings);

			if (settings.Font != null) text.font = settings.Font;
		...
		override public string CheckboxLabel {
			get {
				if (!isInitialized) initialize();
				return checkbox.GetComponentInChildren<Text>().text;
			}
			set {
				if (!isInitialized) initialize();
				checkbox.GetComponentInChildren<Text>().text = value;
